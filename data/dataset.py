...
class dataset():
    ...
    def build(self):
        ...
        train=TranslationDataset(path = self.path, exts = ("train.enc","train.dec"),
                                    fields = datafields)
        test=TranslationDataset(path = self.path, exts = ("test.enc","test.dec"),
                                    fields = datafields)
        val = TranslationDataset(path=self.path, exts=("val.enc", "val.dec"),
                                  fields=datafields)
...
if __name__ == '__main__':
    ...
    data_set = dataset(path='data/', batch_size=128,
                       device=device)
    train_iterator, test_iterator, val_iterator = data_set.build()
